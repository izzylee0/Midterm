		...
		$.ajax({
			...
			success: function(data){
				...
				//for (var i = 0; i < data.objects.length; i++){
					var searchResults = data.objects[0].images[0].b.url; //in future iteration would like to choose a random image from set
	...
	searchlastFM: function(newSearchTerm) {
		...
		$.ajax({
			...
			success: function(data){
				...
				var searchResults = data.results.trackmatches.track[0].name; //in future iteration would like to choose randomly from top 50 tracks 
				console.log(searchResults);
